...
class DBScan_nD:
    ...
    def fit(self, dbscan_data):
        ...
        while len(not_visited_indexes) != 0: # while there are still unvisited points
            ...
            in_cluster_index = [] # elements in the cluster i

            within_eps = dbscan_data[np.sqrt(
                sum([(dbscan_data[i] - point_start[i]) ** 2 for i in range(n_features)])) <= eps].index # get the data that is within eps eclidean distance of the chosen point (close to it)
            ...
            if len(within_eps) >= min_points: # if num points within eps is larger than defined threshold
                ...
                for index in unchecked_in_cluster: # iterate over the unchecked points in cluster i

                    point = [dbscan_data.loc[index, i] for i in range(n_features)] # list of data points

                    visited_indexes.append(index) # has been visted
                    ...
                    unique_indexes = list(
                        set(within_eps) - (set(visited_indexes).union(unchecked_in_cluster))) # remove points that are within eps but are already in the cluster
