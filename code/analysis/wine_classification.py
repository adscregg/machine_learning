...
print('Below the models are being fit with GridSearchCV to find the best hyperparameters for each of our models to ensure that they perform \n the best that they can with the options we have given them, especially as we have a large class imbalance.')
...
print('Fitting models and predicting...', '\n\n')
...
print('Although these results look promising, do not forget that we had a very significant class imbalance which could have affected our \n model. Lets look at the classification report and confusion matrix for each of the models.')
...
print('\n All models still appear to be very good at predicting both classes but there is still some imbalance in the scores, it is noticible \n that class 0 (red) always has slightly lower scores in the classification report which would indicate that the dataset has had some influence on \n each of the models. However, in this case the models still perform well even with the different metrics in the classification report. \n')

print('Following is one possible reason the classifiers still performed quite well. \n')



print('Log: ' + str(log_gs.best_estimator_), '\n\n') # see what parameters the best estimator had from running GridSearchCV)



print('SVC: ' + str(svc_gs.best_estimator_), '\n\n')

print('The two most accurate classifiers have a large C value, this means that they penalize wrong classifications more during the training \n process and hence are \'persuaded\' not to always predict the larger class. Below is an example of fitting our models without tuning the hyper \n parameters.')
...
print('\n It is more noticable that the class difference is having an affect on the overall performance of the models. Note not only the drop \n in accuracy as a measure of performance but the difference of the the values of the metrics between class 0 and class 1, this clearly \n demonstrates that the imbalance is having an impact, albeit not significant in this case as the models are still able to accurately predict the \n class, however, this effect is still one to note when creating models as it may be more significant with other datasets.')
